...
\subsection{Notation and problem statement} %\label{sec:not}
...
\begin{lemma}
  ...
  % and $S'$, and $\measuredangle s_2s_1s_3$ and $\measuredangle s_5s_4s_6$ are the second-largest angles in the triangles formed by $S$ and $S'$.
   If
...
% \subsection{Properties of Steiner minimal trees}
...
%  i.e. if
% \begin{conjecture}
%   Let $S$ and $S'$ be a set of terminals with the same cardinality, allowing no overlapping terminals inside the sets. If the largest angle between any two possible edges with a common terminal in $\operatorname{MST(S')}$ is larger than or equal to the same concept in $\operatorname{MST(S)}$, then $\operatorname{SR}(S)\leq\operatorname{SR}(S')$.
% \end{conjecture}
% \begin{lemma}
%   Let $S=\{s_1, s_2, s_3\}$ and $S'=\{s_4, s_5, s_6\}$, $s_n=\{x_n, y_n\}$ be set of terminals, such that  $\measuredangle s_1s_2s_3$ and $\measuredangle s_4s_5s_6$ are the largest angles in the triangles formed by $S$ and $S'$, and $\measuredangle s_2s_1s_3$ and $\measuredangle s_5s_4s_6$ are the second-largest angles in the triangles formed by $S$ and $S'$. If
%   % $s_1s_2$,  $s_2, s_3$, $s_4, s_5$ and $s_5, s_6$ are all in $\operatorname{MST}(S)$ and $\operatorname{MST}(S')$ as edges,
%   $\measuredangle s_1s_2s_3=\measuredangle s_4s_5s_6$,
%   % and $\allowbreak\frac{\operatorname{len}(s_1s_2)}{\operatorname{len}(s_2s_3)}\allowbreak\leq\allowbreak\frac{\operatorname{len}(s_4s_5)}{\operatorname{len}(s_5s_6)}$,
%   and $\measuredangle s_1s_3s_2 \leq \measuredangle s_4s_6s_5$,
%    then $\operatorname{SR}(S) \allowbreak\geq\allowbreak \operatorname{SR}(S')$
% \end{lemma}
% \begin{proof}
%   As per the triangle inequalities, the largest side in $S$ and $S'$ is $s_1s_3$ and $s_4s_6$, and thus $\operatorname{L_M}(S)=\operatorname{len}(s_1s_2)+\operatorname{len}(s_2s_3)$ and $\operatorname{L_M}(S')=\operatorname{len}(s_4s_5)+\operatorname{len}(s_5s_6)$.

%   $\operatorname{L_S}{S}$ and $\operatorname{L_S}{S'}$ can be calculated using the formula from \cite{uteshev2021length} as 
%   \begin{align*}
%     \operatorname{L_S}{S} =& | x_1+y_1i + (x_2+y_2i)(\cos \frac{2\pi}{3}+\sin \frac{2\pi}{3}i) \\
%     &+ (x_3+y_3i)(\cos \frac{4\pi}{3}+\sin \frac{4\pi}{3}i)| \\
%     =& |x_1+y_1i + (x_2+y_2i)(-\frac{1}{2}+\frac{\sqrt{3}}{2}) \\
%     &+ (x_3+y_3i)(-\frac{1}{2}-\frac{\sqrt{3}}{2})| \\
%     =& |x_1+y_1i + (x_2+y_2i)(-\frac{1}{2}+\frac{\sqrt{3}}{2}) \\
%     &+ (x_3+y_3i)(-\frac{1}{2}-\frac{\sqrt{3}}{2})| \\
%     =& |x_1+y_1i + \frac{-x_2-\sqrt{3}y_2+\left(\sqrt{3}x_2-y_2\right)i}{2} \\
%     &+ \frac{-x_3+\sqrt{3}y_3+\left(-\sqrt{3}x_3-y_3\right)i}{2}| \\
%   \end{align*}
% \end{proof}
% Let us consider the relationship between an angle defined by three terminals and the Steiner minimal tree of said terminals. Let $S=\{A, B, C\}$ where $A$, $B$ and $C$ are terminals, and let $\operatorname{len}(\measuredangle S_1S_2S_3)=\operatorname{len}(S_1S_2)+\operatorname{len}(S_2S_3)$, where $S_1, S_2, S_3$ are unique elements of $S$ let $\operatorname{SR'}(\measuredangle S_1S_2S_3)\allowbreak=\allowbreak\frac{\operatorname{L_S}(S)}{\operatorname{len} (\measuredangle S_1S_2S_3}$ and finally let $S'=\{D, E, F\}$ where $D$, $E$ and $F$ are terminals and let $S_4, S_5, S_6$ are unique elements of $S'$.  Because of triangle similarities, $\operatorname{SR'}(\measuredangle S_1S_2S_3)=\operatorname{SR'}(\measuredangle S_4S_5S_6)$ if and only if $\measuredangle S_1S_2S_3 = \measuredangle S_4S_5S_6$ and $\allowbreak\frac{\operatorname{len}(S_1S_2)}{\operatorname{len}(S_2S_3)}\allowbreak=\allowbreak\frac{\operatorname{len}(S_4S_5)}{\operatorname{len}(S_5S_6)}$
% %  or $\frac{\operatorname{len}(S_1S_2)}{\operatorname{len}(S_2S_3)}=\frac{\operatorname{len}(S_5S_6)}{\operatorname{len}(S_4S_5)}$.
% assuming $\operatorname{len}(S_1S_2)\leq\operatorname{len}(S_2S_3)$ and $\operatorname{len}(S_4S_5)\allowbreak\leq\operatorname{len}(S_5S_6)$. With this is in mind, let us prove the following:
% \begin{lemma}
%   If $\operatorname{len}(S_1S_2)\leq\operatorname{len}(S_2S_3)$ and $\allowbreak\operatorname{len}(S_4S_5)\allowbreak\leq\allowbreak\operatorname{len}(S_5S_6)$
%   and $\measuredangle S_1S_2S_3 = \measuredangle S_4S_5S_6$ and $\allowbreak\frac{\operatorname{len}(S_1S_2)}{\operatorname{len}(S_2S_3)}\allowbreak\leq\allowbreak\frac{\operatorname{len}(S_4S_5)}{\operatorname{len}(S_5S_6)}$, then $\operatorname{SR'}(\measuredangle S_1S_2S_3)\allowbreak\geq\allowbreak\operatorname{SR'}(\measuredangle S_4S_5S_6)$.
% \end{lemma}
% \begin{proof}
%   As per the law of cosines, the two edges in $\operatorname{SMT}(S)$ connecting $S_1$ and $S_2$
% \end{proof}
% The length of the angle the sum of the length of the two line segments used to define the angle; let us measure the ratio between the length of the angle and the length of the Steiner minimal tree as $SR'(S)$. Because of triangle similarities, the all angles are similar to $S$ if the value of the angle and the ratio between the sides is the same as the value of the angle and the ratio between the sides of $S$.
%  is minimal if exactly two terminals have the same coordinates and the angle is $0\pi$, where $SR'(S)=\frac{1}{2}$.

% For the case of three terminals, let us measure the ratio of Steiner minimal tree and the perimeter of the triangle $P_{\bigtriangleup}$(if $a, b$ and $c$ are the sides of the triangle then $P_{\bigtriangleup}=a+b+c$, even if all 3 terminals lie on the same line) as $SR'$. The lowest $SR'$ is achieved when all three points lie on the same line, and the highest $SR'$ is achieved in the case of equilateral triangle. 


% \subsection{Equilateral triangles}
% As we have seen in Section \ref{sec:mla}, equilateral triangles play a huge role in finding low Steiner ratio instances. This subsection will explore other explanations as to why equilateral triangles yield so low 
...
\end{figure}
\subsection{Proof for Subsection \ref{sec:pgm}}
...
% TODO: remark maybe?
% \begin{proof}
%   Let us prove this by contradiction; for $2k=4$, we already proved that this is the Steiner minimal tree topology, so we will only be considering the cases $k\geq 3$.
